      ...
      >
        <view
          ...
          :style="{
            ...
            flex: scroll ? '' : 1,
            padding: paddingItem,
						backgroundColor: current == i ? activeBgColor : itemBgColor
          }"
          @click="change(i)"
        >
          {{ field ? v[field] : v }}
					<text v-if="v.showBadge && v.count > 0" class="badge">{{v.count}}</text>
        </view>
        <view
          v-if="!pills"
 ...
 * @property {String} activeBgColor = '#fff' 当前选中背景
 * @property {String} itemBgColor = '#fff' tab item背景
 *
